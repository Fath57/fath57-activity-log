export function getFeedSchemaStatements(): string[] {
  return [
...
export function getAuditSchemaStatements(): string[] {
  return [
...
export function getDropFeedSchemaStatements(): string[] {
  return [
    `DROP TABLE IF EXISTS public.activity_outbox CASCADE;`,
    `DROP TABLE IF EXISTS public.activity_logs CASCADE;`,
  ];
}

export function getDropAuditSchemaStatements(): string[] {
  return [`DROP SCHEMA IF EXISTS audit CASCADE;`];
}

export function getHardeningScript(
  appUser = 'app_user',
  auditAdmin = 'audit_admin',
): string {
  return `-- Schema ownership FIRST.
...
-- on the first audited write after hardening. Caught by
-- partition-privileges.integration.spec.ts.
ALTER SCHEMA   audit                                                   OWNER TO ${auditAdmin};

-- Ownership transfers
ALTER TABLE    audit.logged_actions                                    OWNER TO ${auditAdmin};
...
GRANT  USAGE ON SCHEMA audit           TO   ${auditAdmin};
GRANT  USAGE ON SCHEMA audit           TO   ${appUser};
REVOKE ALL   ON audit.logged_actions   FROM PUBLIC, ${appUser};
GRANT  SELECT ON audit.logged_actions  TO   ${appUser};
...
BEGIN
    FOR r IN
        ...
        WHERE  i.inhparent = 'audit.logged_actions'::regclass
    LOOP
        EXECUTE format('ALTER TABLE %s OWNER TO ${auditAdmin};', r.part);
        EXECUTE format('REVOKE ALL ON %s FROM PUBLIC, ${appUser};', r.part);
    END LOOP;
END $$;
`;
}
...
export function getTrackTableSql(
  ...
  captureQuery: boolean = false,
): string {
  const pkSql = `ARRAY[${pkColumns.map((c) => `'${c}'`).join(', ')}]::TEXT[]`;
  const ignoredSql = `ARRAY[${ignoredColumns.map((c) => `'${c}'`).join(', ')}]::TEXT[]`;
  return `SELECT audit.track_table('${targetTable}'::REGCLASS, ${pkSql}, ${ignoredSql}, ${captureQuery});`;
}
